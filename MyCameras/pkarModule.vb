...
#Region "Back button"
    ...
    Private Sub OnBackButtonPressed(sender As Object, e As Windows.UI.Core.BackRequestedEventArgs)
        Try
            ...
            e.Handled = True
        Catch ex As Exception
            CrashMessageExit("@OnBackButtonPressed", ex.Message)
        End Try
    End Sub
...
#Region "CrashMessage"
    ...
    Public Sub CrashMessageInit()
        ...
        mAppName = Package.Current.DisplayName
    End Sub
    ...
    Public Async Function CrashMessageShow() As Task
        ...
        Try
            Await DialogBox(mAppName & " fail messages:" & vbCrLf & sTxt)
            sTxt = ""
        Catch ex As Exception

        End Try

        If sTxt <> "" Then Await DialogBox("ERROR showing CrashMessages")
        SetSettingsString("appFailData", "")
    End Function

    Public Sub CrashMessageAdd(sTxt As String, ex As Exception)
    ...
    Public Sub CrashMessageAddOneDrive(sTxt As String, ex As Exception)
        Dim sMsg As String = ex.Message
        If ex.StackTrace IsNot Nothing Then
            sMsg = sMsg & vbCrLf & ex.StackTrace
        End If

        CrashMessageAddOneDrive(sTxt, sMsg)

    End Sub

    Public Sub CrashMessageAddOneDrive(sTxt As String, exMsg As String)
        Dim sMsg As String = sTxt & vbCrLf & exMsg
        Dim sAdd As String = Date.Now.ToString("HH:mm:ss") & " " & sTxt & vbCrLf & exMsg
...
#End If
        ...
        If GetSettingsBool("settIgnoreODerror") Then Return

        MakeToast(mAppName & "@" & mHostName & ", " & Date.Now.ToString("HH:mm:ss"), sMsg)

        SetSettingsString("appFailData", GetSettingsString("appFailData") & sAdd & vbCrLf)
    End Sub
...
#End Region
    ...
    Public Sub SetBadgeNo(iInt As Integer)
        ...
        Dim oXmlBadge = Windows.UI.Notifications.BadgeUpdateManager.GetTemplateContent(
                Windows.UI.Notifications.BadgeTemplateType.BadgeNumber)
        Dim oXmlNum = CType(oXmlBadge.SelectSingleNode("/badge"), Windows.Data.Xml.Dom.XmlElement)
...
' ****************************************************************************
'
'           JASNOSC        JASNOSC        JASNOSC        JASNOSC        JASNOSC
'
' *******************************************************************************


Public Module Jasnosc

    Private goAmbientLightSensor As Windows.Devices.Sensors.LightSensor = Nothing
    Private gdLastAmbientLight As Single = -1
    Public Event ZmianaJasnosci()

    Public Function JasnoscGetLast() As Single
        Return gdLastAmbientLight
    End Function

    Public Function JasnoscJakJasno() As Integer
        If goAmbientLightSensor Is Nothing Then
            JasnoscStart()
            If goAmbientLightSensor Is Nothing Then Return -1
        End If

        Try
            Dim oLD As Windows.Devices.Sensors.LightSensorReading = goAmbientLightSensor.GetCurrentReading
            If oLD Is Nothing Then Return -1
            Return oLD.IlluminanceInLux
        Catch ex As Exception
            Return -1
        End Try
    End Function

    Private Function JasnoscKontrolaZmiany(dCurrVal As Single) As Boolean
        ' sprawdzenie czy oznaczyc jako zmiana jasnosci?
        Dim iLastAmbientLight As Single = gdLastAmbientLight
        ' .giLastAmbientLight = App.JakJasno()
        gdLastAmbientLight = dCurrVal
        If gdLastAmbientLight = -1 Then Return False
        'Debug.WriteLine("jasnosc poprzednia: " & iLastAmbientLight & ", aktualna " & .giLastAmbientLight)
        If iLastAmbientLight * 0.6 > gdLastAmbientLight Then Return True
        If iLastAmbientLight * 1.4 < gdLastAmbientLight Then Return True

        Return False
    End Function

    Public Sub JasnoscStart()
        If goAmbientLightSensor IsNot Nothing Then Exit Sub

        goAmbientLightSensor = Windows.Devices.Sensors.LightSensor.GetDefault()
        If goAmbientLightSensor Is Nothing Then Return
        goAmbientLightSensor.ReportInterval = 250   ' 250 ms
        AddHandler goAmbientLightSensor.ReadingChanged, AddressOf JasnoscZmianaEvent
    End Sub

    Public Sub JasnoscStop()
        If goAmbientLightSensor IsNot Nothing Then
            RemoveHandler goAmbientLightSensor.ReadingChanged, AddressOf JasnoscZmianaEvent
        End If
        goAmbientLightSensor = Nothing
    End Sub

    Private Sub JasnoscZmianaEvent(sender As Windows.Devices.Sensors.LightSensor, args As Windows.Devices.Sensors.LightSensorReadingChangedEventArgs)
        If JasnoscKontrolaZmiany(args.Reading.IlluminanceInLux) Then
            RaiseEvent ZmianaJasnosci()
        End If
    End Sub
