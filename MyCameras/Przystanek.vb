...
' skopiowane z KrakTram, plus dodanie odleglosci
...
Public Class Przystanek
    ...
    ' te, ktore są dodane w MyCameras
    <XmlIgnore>
    Public Property dOdl As Double = 99999
    <XmlIgnore>
    Public Property bChecked As Boolean = False
    <XmlIgnore>
    Public Property sTyp As String = "T"
End Class
...
Public Class Przystanki
    ...
    Public Sub DodajOdleglosci(oPoint As Point, sChecked As String)
        ' mozna by dokleic to od razu do czytania, ale tak jest mniej zmian przy przechodzeniu KrakTram/Kamerki
        For Each oItem As Przystanek In moItemy
            oItem.dOdl = GPSdistanceDwa(oPoint, oItem.Lat, oItem.Lon)
            oItem.sTyp = If(oItem.Cat = "bus", "A", "T")
            oItem.bChecked = sChecked.Contains(oItem.sTyp & oItem.id & "#")
        Next
    End Sub
    ...
    Private Async Function Import() As Task(Of Boolean)
        ...
        If Not Net.NetworkInformation.NetworkInterface.GetIsNetworkAvailable() Then
            DialogBoxRes("errCannotWorkNoNet")
            Return False
        End If
